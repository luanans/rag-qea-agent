...
### Estratégia de chunking: Chunking Hierárquico
...
Os testes implementados têm caráter **orientador e não exaustivo** — foram suficientes para guiar as decisões do projeto e validar os contratos entre as camadas, mas não substituem uma suíte de testes completa e criteriosa em um contexto de produção.

A pasta `eval/` contém dois notebooks de validação executados durante o desenvolvimento:

**`embedding_benchmark.ipynb`** — compara quatro modelos de embedding (`all-MiniLM-L6-v2`, `bge-large-en-v1.5`, `e5-large-v2` e `gemini-embedding-001`) usando as métricas do MTEB. Foi esse benchmark que fundamentou a escolha do `gemini-embedding-001` como modelo de produção.

**`rag_evaluation_v2.ipynb`** — valida o pipeline RAG completo usando a biblioteca RAGAS em um conjunto reduzido de 10 perguntas, suficiente para validar o processo sem incorrer em custos elevados de API. Os resultados obtidos foram:
...
Os notebooks não fazem parte da suíte de testes automatizados — são artefatos de análise exploratória e servem de evidência das decisões técnicas tomadas.
...
O `chunk_size` fixo demonstrou bom desempenho dentro do escopo do projeto. No entanto, mesmo com o uso de overlapping, em cenários mais complexos ainda existe a possibilidade de perda de contexto. A evolução para um chunking semântico (baseado na variação de significado entre sentenças) é o próximo passo lógico. Essa solução foi preterida inicialmente pela simplicidade e velocidade de aplicar o chunking fixo dentro das seções já estruturadas pelo Docling.
...
O RAG atual é do tipo denso, baseado na similaridade de cosseno de embeddings. Nos testes realizados, essa abordagem demonstrou resultados satisfatórios. Entretanto, para sistemas que enfrentam falhas na busca de termos técnicos muito específicos ou siglas raras, recomenda-se uma abordagem híbrida (hybrid retrieval), combinando a busca vetorial (semântica) com a busca por palavras-chave (BM25/keyword search).
...
O sistema atual não tem tracing. A integração de ferramentas como LangSmith, Langfuse ou OpenTelemetry é essencial para monitorar custos e latência em produção.
